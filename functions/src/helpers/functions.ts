...
export const onRequest = (options: onRequestOptions) => 
    functions.region("asia-northeast3").https.onRequest(async (request, response) => {
        logger.log(`Running onRequest ${options.name}`);
        let method = METHOD.GET;
        let methodVerb = request.method;
        ...
        try {
            if (options.schema[methodVerb]) {
                logger.info("Validating data...");
                const validationResult = options.schema[methodVerb].validate(request.body);
                if (validationResult.error) {
                  logger.error("Validation Error", validationResult.error);
                  throw validationResult.error;
                }
