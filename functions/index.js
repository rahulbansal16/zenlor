...
const generateTaskId = (taskName) => {
  ...
  let filtered = words.map(getAlphaCharacters).filter( word  => word.length >=1)
...
exports.completedTasks = functions.region('asia-northeast3').https.onCall(async (data, context) => {
...
const removeDependentTask = (tasks) => {
  ...
  return tasks.filter( task => {
    ...
    return dependent[taskId].every(item => taskStatus[item] === "complete" || taskStatus[item] === undefined)
  })
}
